...
import { extractMethodDto, extractResponseType } from './type.extractor';

const HTTP_METHODS = ['Get', 'Post', 'Put', 'Delete', 'Patch', 'Options', 'Head'];

export function extractEndpoints(classDecl: ClassDeclaration, controllerPath: string, moduleName: string): ApiEndpoint[] {
  ...
  for (const method of methods) {
    for (const decorator of method.getDecorators()) {
      ...
      if (HTTP_METHODS.includes(decoratorName)) {
        ...
        const dtos = extractMethodDto(method);
        ...
        endpoints.push({
          ...
          response: extractResponseType(method),
...
function extractGuards(node: MethodDeclaration | ClassDeclaration): string[] {
  const guards: string[] = [];
...
function extractRoles(node: MethodDeclaration | ClassDeclaration): string[] {
  const roles: string[] = [];
...
function extractSwaggerTags(node: ClassDeclaration): string[] {
  const tags: string[] = [];
...
function extractSwaggerSummary(node: MethodDeclaration): string | undefined {
...
function extractSwaggerResponses(node: MethodDeclaration): Array<{ status: number | string; description?: string; type?: string }> {
  const responses: Array<{ status: number | string; description?: string; type?: string }> = [];
  for (const decorator of node.getDecorators()) {
    if (decorator.getName() === 'ApiResponse' || decorator.getName().startsWith('Api') && decorator.getName().endsWith('Response')) {
      ...
      let status: number | string = 'default';
      if (name === 'ApiOkResponse') status = 200;
      else if (name === 'ApiCreatedResponse') status = 201;
      else if (name === 'ApiAcceptedResponse') status = 202;
      else if (name === 'ApiNoContentResponse') status = 204;
      else if (name === 'ApiBadRequestResponse') status = 400;
      else if (name === 'ApiUnauthorizedResponse') status = 401;
      else if (name === 'ApiForbiddenResponse') status = 403;
      else if (name === 'ApiNotFoundResponse') status = 404;
      else if (name === 'ApiInternalServerErrorResponse') status = 500;
