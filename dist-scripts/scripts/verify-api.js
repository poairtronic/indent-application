    ...
    for (let i = 2; i < args.length; i++) {
        switch (args[i]) {
            ...
            case '--no-archive':
                ...
                break;
            case '--p0-threshold':
                opts.p0Threshold = parseInt(args[++i], 10);
                break;
            case '--p1-threshold':
                opts.p1Threshold = parseInt(args[++i], 10);
                break;
            case '--p2-threshold':
                opts.p2Threshold = parseInt(args[++i], 10);
                break;
            case '--p3-threshold':
                opts.p3Threshold = parseInt(args[++i], 10);
                break;
            case '--swagger-url':
                opts.swaggerUrl = args[++i];
                break;
            case '--backend-url':
...
function banner(step, total, title) {
    console.log('');
...
function success(msg) { console.log(`${c.bold}${c.green}  ✔ ${msg}${c.reset}`); }
function warn(msg) { console.log(`${c.bold}${c.yellow}  ⚠ ${msg}${c.reset}`); }
function fail(msg) { console.log(`${c.bold}${c.red}  ✖ ${msg}${c.reset}`); }
function info(msg) { console.log(`${c.dim}  ℹ ${msg}${c.reset}`); }
function heading(msg) { console.log(`\n${c.bold}${c.white}  ${msg}${c.reset}`); }
function runCommand(cmd, cwd, label) {
    try {
        (0, child_process_1.execSync)(cmd, { cwd, stdio: 'pipe', timeout: 120000 });
        return true;
    }
    catch (err) {
        ...
        fail(`${label} failed:`);
        if (stderr)
            console.log(`    ${c.red}${stderr.split('\n').slice(0, 5).join('\n    ')}${c.reset}`);
        if (stdout)
            console.log(`    ${c.dim}${stdout.split('\n').slice(0, 5).join('\n    ')}${c.reset}`);
        return false;
    }
}
// ─── Step 1: Backend Scanner ────────────────────────────────────────────────
function step1_backendScanner() {
    ...
    const cmdOk = runCommand(`npx ts-node src/index.ts -p "${path.join(ROOT, 'backend/tsconfig.json')}" -o "${reportsRel}"`, BACKEND_SCANNER, 'Backend Scanner');
...
function step2_frontendScanner() {
    ...
    const ok = runCommand(`npx ts-node src/index.ts -p "${path.join(ROOT, 'frontend/tsconfig.app.json')}" -o "${reportsRelFe}"`, FRONTEND_SCANNER, 'Frontend Scanner');
...
function step3_verificationEngine(opts) {
    ...
    `;
        ...
        const ok = runCommand(`npx ts-node src/index.ts -b "${backendApi}" -f "${frontendApi}" -o "${reportsRelVe}"`, VERIFICATION_ENGINE, 'Verification Engine');
...
function step4_swaggerVerification(opts) {
    ...
    `;
        ...
        const result = (0, child_process_1.execSync)(`node "${scriptPath}"`, {
            cwd: ROOT,
            timeout: 30000,
            encoding: 'utf-8',
...
function generateSwaggerComparisonMd(report) {
...
function generateOpenApiMismatchMd(report) {
    let md = `# OpenAPI Discrepancies\n\n`;
    md += `> Generated at ${report.generatedAt}\n\n`;
    const missingBackend = report.matrix.filter((r) => r.result === 'MISSING_IN_BACKEND');
...
function step5_runtimeVerification(opts) {
...
function generateRuntimeReport(report) {
...
// ─── Final Summary ──────────────────────────────────────────────────────────
function finalSummary(results, opts) {
    console.log('');
    console.log(`${c.bold}${c.cyan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${c.reset}`);
