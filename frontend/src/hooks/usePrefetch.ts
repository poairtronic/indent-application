...
export function usePrefetch() {
  ...
  const prefetchPath = (path: string) => {
    if (path.includes('/dashboard')) {
      ...
      import('../api/services/analytics/service').then(m => {
    ...
    if (path.includes('/indents') && !path.includes('create')) {
      ...
      import('../api/services/indents/service').then(m => {
        queryClient.prefetchQuery({
      ...
      import('../api/services/materials/service').then(m => {
        queryClient.prefetchQuery({
      ...
      import('../api/services/products/service').then(m => {
        queryClient.prefetchQuery({
      ...
      import('../api/services/departments/service').then(m => {
        queryClient.prefetchQuery({
    ...
    if (path.includes('/workflow')) {
      ...
      import('../api/services/indents/service').then(m => {
        queryClient.prefetchQuery({
          queryKey: [...queryKeys.indents.list('indents'), { state: 'DESIGN_COMPLETED,STORES_PROCESSING' }],
          queryFn: () => m.indentService.list({ state: 'DESIGN_COMPLETED,STORES_PROCESSING' as any }),
          staleTime: 60 * 1000,
        });
      }).catch(() => {});
    }
